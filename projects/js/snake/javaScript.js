...
function createFood() {
	...
	foodEl.id = "food";
	document.getElementById("game").appendChild(foodEl);
}
...
function moveSnakeBody() {
	...
	for (let i = 0; i < snakeBody.length; i++) {
		...
		document.getElementById("game").appendChild(snakeBodyPart);
	}
}
...
function checkGameOver() {
	...
	let gameAreaRect = document.getElementById("game").getBoundingClientRect();
	if (snakeRect.top < gameAreaRect.top || snakeRect.bottom > gameAreaRect.bottom) {
		clearInterval(gameInterval);
		alert("Game Over! Your score: " + score);
		resetGame();
		return;
	} else if (snakeRect.right > gameAreaRect.right) {
