...
function hh(color, id) {
	if (color == "" && id == "a") {
		let random_board = enableRandomBoard();
		setBoardColor(random_board, "light");
	}
}
...
let applyColorAndDisableCells = (disableCellsListOfIds, lightCellId) => {
	let donish = done;
	try {
		var setBoardColors = setBoardColor(lightCellId, "light");
		enableBoard(lightCellId);

		for (let boardId of disableCellsListOfIds) {
			if (boardId == lightCellId && setBoardColors == "dark" && donish != lightCellId) {
				checkWinner();
			} else {
...
//     let aa = enableRandomBoard()
// alert(aa);
// setBoardColor(aa,'danger')
